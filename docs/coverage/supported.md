...
| **Local (rooftop) rates** | ❌ not resolved. The dataset carries every local record, but the shipped `GeocodioGeocoder` resolves **state-level only**, and the experimental `us_tax_data.rooftop` path does not yet match a record — [why](us-tax-dataset.md#rooftop-is-partial-and-opt-in) | a locality code in each state's own key shape, **plus** the boundary data that says which local records apply at an address (inside Kansas City a county and a city record both apply; inside Seattle only the city one does). Absent a resolved locality the **state rate** applies, so city/district rates are not included |
