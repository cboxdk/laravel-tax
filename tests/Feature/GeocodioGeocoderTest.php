...
it('resolves an address to a jurisdiction with its subdivision', function (): void {
    $http = new Factory;
...
it('targets the v2 endpoint by default', function (): void {
    $http = new Factory;
    $http->fake([
        'api.geocod.io/*' => $http->response([
            'results' => [
...
it('still reads the v1 `state` key so a pinned older baseUrl keeps resolving', function (): void {
    $http = new Factory;
    $http->fake([
        'api.geocod.io/*' => $http->response([
...
it('denies by default on an empty geocoding result', function (): void {
    $http = new Factory;
    $http->fake([
...
it('attaches a ZIP+4 locality from the zip4 append when rooftop is enabled', function (): void {
...
it('does not attach a locality when rooftop is disabled (the default)', function (): void {
    $http = new Factory;
    $http->fake([
        'api.geocod.io/*' => $http->response([
            'results' => [
...
it('refuses an ambiguous ZIP+4 rather than picking one', function (): void {
...
it('requests the zip4 append, not census, when rooftop is enabled', function (): void {
    $http = new Factory;
    $http->fake(['api.geocod.io/*' => $http->response(['results' => [
        ['address_components' => ['country' => 'US', 'state_province' => 'CA']],
    ]])]);
...
it('retries a transient failure before giving up', function (): void {
...
it('still denies when the failure persists', function (): void {
    $http = new Factory;
...
it('attaches a point locality for the states resolved by polygon', function (): void {
...
it('keeps the ZIP+4 key for states resolved by the boundary index', function (): void {
    $http = new Factory;
    $http->fake([
        'api.geocod.io/*' => $http->response([
            'results' => [
...
it('asks the installed register which states need only the county, not a list in the engine', function (): void {
    ...
    $locate = function (string $state, string $county): ?Jurisdiction {
        $http = new Factory;
        $http->fake(['api.geocod.io/*' => $http->response(['results' => [[
...
it('carries the point with the ZIP+4 in a state that draws districts over its postal layer', function (): void {
