...
it('treats SaaS as taxable in states that clearly tax it', function (string $state) {
    expect($this->taxability->determine(usPlace($state), TaxClass::DigitalService, anyAmount())->isExemptFor(anyAmount()))->toBeFalse();
...
it('treats SaaS as exempt in states that clearly exempt it', function (string $state) {
    expect($this->taxability->determine(usPlace($state), TaxClass::DigitalService, anyAmount())->isExemptFor(anyAmount()))->toBeTrue();
...
it('leaves undetermined states absent and refuses to guess', function (string $state) {
    ...
    expect($overrides)->not->toHaveKey($state.':'.TaxClass::DigitalService->value);

    $this->taxability->determine(usPlace($state), TaxClass::DigitalService, anyAmount());
...
it('does not touch tangible-goods (standard) taxability', function () {
    ...
    expect($this->taxability->determine(usPlace('US-CA'), TaxClass::GeneralGoods, anyAmount())->isExemptFor(anyAmount()))->toBeFalse();
});

it('is the bound default ProductTaxability', function () {
    $bound = $this->app->make(ProductTaxability::class);

    expect($bound->determine(usPlace('US-CA'), TaxClass::DigitalService, anyAmount())->isExemptFor(anyAmount()))->toBeTrue()
        ->and($bound->determine(usPlace('US-NY'), TaxClass::DigitalService, anyAmount())->isExemptFor(anyAmount()))->toBeFalse();
});
